...
class SearchConfig:
    ...
    max_trials: int = 96
    max_depth: int = 1
    # Two budgets, deliberately not one. A ninth conformer of a known substance is not
    # a chemical discovery and must not consume a chemical node slot; but conformers
    # stay searchable, because reaction accessibility is conditional on conformer.
    ...
    # The default basis spans stretch, bend and torsion. A basis of pair directions
    # alone cannot reach a torsional basin at any amplitude.
    families: tuple[str, ...] = ("stretch", "compress", "kick", "bend",
                                 "torsion", "torsion_kick")
    ...
    # stationary: |dq| ~ |g_q| / k_q. Measured on ethanol, the hydroxyl torsion is
    # 0.21 eV/rad^2 against 33-53 eV/A^2 for the bond stretches, so one tolerance cannot
    # serve both. Soft coordinates get their own criterion instead of dragging the global
    # one down to a precision the stiff coordinates do not need.
    ...
    # not flat -- the Newton scale is simply unusable there. Its own path is an exact rigid
    # rotation, so the symmetry-reduced period is scanned instead of trusting a local model
    # that has just been shown not to hold. Below three points no scan is done and the
    # coordinate is polished by capped steps alone, which is cheaper and less reliable.
    ...
    # alternations to settle -- its curvature rose from 0.0041 eV/rad^2 at the inflection to
    # 0.107 at the minimum, so the first steps are taken on a scale 25 times too small.
    ...
    # sqrt(kT/kappa) is the physical anchor: about 0.3 A*sqrt(amu) for a soft mode at 300 K.
    ...
    # Finding the unstable direction is not finding the saddle; the centre has to move too.
    ...
    # happens. Separate from the trial budget: it spends force evaluations, not trials, and
    # the seed comes from work already done.
    saddle_search_enabled: bool = True
    saddle_search_per_run: int = 8
    saddle_search_seeds: int = 2
    saddle_search_max_rise_eV: float = 1.5
    # Refuse endpoints with no neutral closed-shell valence assignment. MACE-OFF is trained
    # on neutral closed-shell molecules and ignores total charge outright (section 2.5), so
    # a radical pair reached by an over-driven probe is not a product it can describe.
    ...
    # and its two-sided descent establish that two minima are joined; they do not establish
    # that the probe that was delivered reaches the product, and those are different claims.
    # The continuation answers the second one by staying on the probe's own tangent and
    # quenching each rung independently: a rung that returns to the source says the
    # crossing had not yet entered the product's basin, a rung that reaches another
    # chemical state is a directed observation, and a rung that stops on a saddle again
    # says the delivered path runs along the dividing surface.
    boundary_continuation_enabled: bool = True
    # A cap on the largest single-atom displacement per rung, not an RMS. Small on purpose:
    # a long step can cross several dividing surfaces and the rung's verdict would then name
    # a basin the probe never passed through.
    ...
    # the same chemical state then every neighbourhood of that rung except the ridge belongs
    # to that state. Measured on 3-oxobutanal round four, every rung of every continuation
    # stopped on a rotor saddle 11 meV above the product minimum at -65 to -79 cm^-1 while
    # the proton had crossed. Costs two quenches per stalled rung and is reported as a chain.
    ...
    # A pair-approach probe between atoms two or three bonds apart is a bend or a torsion in
    # disguise: the covalent skeleton already holds them, so the continuation must drag the
    # connecting bonds and the collateral gate refuses the delivery. Measured on
    ...
    # the reactant's own geometry, so it goes in the front band. This is an ordering, never
    # a filter, and it reads nothing about the product.
    ...
    # None means every atom. A subset scopes graphs, RMSD and basin identity to a
    # reactive region so an environment cannot masquerade as a new basin.
    active_atoms: tuple[int, ...] | None = None

    def __post_init__(self):
        if not isinstance(self.irc_enabled, bool):
            raise ValueError("irc_enabled must be a bool")
        integer_positive = ("max_trials", "max_directions", "response_steps",
                            "irc_max_per_run",
                            "boundary_continuation_max_steps",
                            "boundary_continuation_seeds",
                            "boundary_continuation_per_run",
                            "sample_interval", "quench_steps", "chemical_max_nodes",
                            "chemical_trials_per_node", "conformer_max_per_node",
                            "conformer_trials_per_microstate", "automorphism_limit",
                            "response_samples")
        integer_nonnegative = ("seed", "max_depth", "refinement_steps", "pulse_steps",
                               "minimum_check_probes", "soft_polish_rounds",
        ...
        for name in ("collateral_gate", "response_band_enabled",
                     "saddle_search_enabled", "closed_shell_only",
                     "boundary_continuation_enabled",
                     "boundary_continuation_descend_saddles"):
            if not isinstance(getattr(self, name), bool):
                raise ValueError(f"{name} must be a bool")
        if not 0 < self.soft_mode_curvature_floor_eV_rad2 < self.soft_mode_curvature_max_eV_rad2:
            raise ValueError("soft-mode curvature floor must be positive and below the maximum")
        for name in ("irc_step_A", "torsion_tolerance_rad",
                     "soft_mode_curvature_max_eV_rad2",
                     "soft_mode_curvature_floor_eV_rad2", "soft_mode_step_rad",
                     "torsion_symmetry_margin_rad", "collateral_bond_A", "timestep_fs",
                     "quench_fmax_eV_A",
                     "minimum_check_displacement_A",
                     "minimum_check_step_A", "minimum_check_eigenvalue_tol",
                     "pair_cutoff_A", "hbond_acceptor_A", "hbond_angle_deg",
                     "direction_quota_fraction", "bond_scale",
                     "basin_rmsd_A", "basin_energy_eV", "min_distance_ratio",
                     "analytic_hessian_check", "trivial_floor_fraction",
                     "response_significance", "response_descent_step",
                     "response_descent_tol", "min_mode_step_A",
                     "min_mode_fmax_eV_A", "min_mode_overlap",
                     "saddle_search_max_rise_eV",
                     "boundary_continuation_step_A",
                     "max_force_eV_A", "max_energy_rise_eV_atom", "max_nve_drift_eV_atom"):
            value = getattr(self, name)
            if isinstance(value, bool) or not math.isfinite(value) or value <= 0:
                raise ValueError(f"{name} must be finite and positive")
        for name in ("reservoir_weight_energy", "reservoir_weight_diversity",
                     "reservoir_weight_response"):
            value = getattr(self, name)
            if isinstance(value, bool) or not math.isfinite(value) or value < 0:
                raise ValueError(f"{name} must be finite and nonnegative")
        if sum((self.reservoir_weight_energy, self.reservoir_weight_diversity,
                self.reservoir_weight_response)) <= 0:
        ...
        if not self.charge_sensitive and self.total_charge not in (None, 0):
            raise ValueError("charge_sensitive is false, so total_charge cannot be set; "
                             "the potential does not respond to it")
        for name in ("temperature_K", "conformer_temperature_K", "bond_hysteresis"):
            value = getattr(self, name)
            if isinstance(value, bool) or not math.isfinite(value) or value < 0:
                raise ValueError(f"{name} must be finite and nonnegative")
        ...
        for name in ("geometry_amplitudes_A", "kick_energies_eV", "pulse_forces_eV_A",
                     "bend_amplitudes_rad", "torsion_amplitudes_rad"):
            values = tuple(getattr(self, name))
            if not values or any(isinstance(v, bool) or not math.isfinite(v) or v <= 0 for v in values):
        ...
        for spec in tracked:
            if set(spec) != {"name", "kind", "indices"}:
                raise ValueError("tracked_coordinates entries need exactly "
                                 "name, kind and indices")
            ...
            if len(indices) != widths[spec["kind"]] or any(i < 0 for i in indices):
                raise ValueError(f"{spec['kind']} needs {widths[spec['kind']]} "
                                 "nonnegative atom indices")
            ...
            if spec["kind"] == "bond_difference":
                if indices[0] == indices[1] or indices[2] == indices[3]:
                    raise ValueError("each bond of a bond_difference needs two "
                                     "distinct atoms")
