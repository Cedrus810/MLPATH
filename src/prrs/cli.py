...
def export_path(run, attempt, output, images):
    ...
    arc = np.concatenate(
        [
            ...
            np.cumsum(
                np.linalg.norm(
                    np.diff(coordinates, axis=0).reshape(len(frames) - 1, -1), axis=1
                )
            ),
        ]
    )
    ...
    interpolated = np.column_stack(
        [np.interp(targets, arc, flat[:, i]) for i in range(flat.shape[1])]
    )
    result = []
...
def main(argv=None):
    ...
    demo = sub.add_parser(
        "demo", help="Offline analytic double-well benchmark (not chemical validation)"
    )
    demo.add_argument("--output", required=True)
    demo.add_argument("--seed", type=int, default=42)
    search = sub.add_parser(
        "search", help="Search with an explicitly trusted ASE calculator factory"
    )
        ...
        if args.command == "demo":
            ...
            result = ReactionSearch(
                ...
                config,
                {
                    ...
                    "purpose": "synthetic software benchmark",
                },
            ).run(demo_atoms(), args.output)
        elif args.command == "search":
            config = (
                SearchConfig.from_dict(load_json(args.config))
                if args.config
                else SearchConfig()
            )
            ...
            print(
                json.dumps(
                    export_path(args.run, args.attempt, args.output, args.images), indent=2
                )
            )
            return 0
        print(
