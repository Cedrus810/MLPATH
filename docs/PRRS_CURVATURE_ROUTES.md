...
> **勘误 · 2026-09-24（原文保留）**：这一步只在 $a \to 0$（$\kappa_a \to v^\top H v$，二次型）时成立。
> 有限 $a$ 下 $\kappa_a$ 不是二次型：负值方向集合可以是锥而不是子空间，二阶鞍的第二个不稳定方向
> 可被四次项读成正，而 argmin 实际由局部优化给出。有限幅度响应只能给方向排序，**不能**给负模数；
> 负模数只由投影 Hessian 判（代码现在就是这么做的）。见 [REVIEW_2026-09-24.md](REVIEW_2026-09-24.md) §1。
...
> **勘误 · 2026-09-24（原文保留）**：上框两处不成立。(i) 对**采到的**方向逐个要求 $\hat\kappa_j > 0$
> 排除不了没采到的负模；即使方向张满全空间，对角元全正也推不出正定（$H = [[1,2],[2,1]]$）。
> (ii) $\sigma$ 是确定性拟合的 OLS 残差，不是误差棒 —— `runner.py` 响应拟合的 Notes [2] 实测
> $\sigma = 3.7\times10^{-6}$ 而值错 0.18，所以 $3\sigma$ 检验没有统计意义。
> 见 [REVIEW_2026-09-24.md](REVIEW_2026-09-24.md) §1。
