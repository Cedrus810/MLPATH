"""ARCHIVED DESIGN — never wired in. Not part of the package.

This was the engine-neutral execution contract from PRRS_ENGINEERING_PLAN.md section 5
(ModelSpec / PotentialBackend / DynamicsBackend). It lived in src/prrs/engine.py from the
start and nothing ever imported it: not the package, not the tests, not a script. It also
defined a second GateRejected, structurally different from the one in prrs/reliability.py
that the search actually raises and catches, so the package carried two same-named
exceptions with different attributes.

It was moved out of the package on 2026-09-04 because an unused abstraction inside src/
reads as an abstraction that is in use -- it suggests the runner speaks to backends
through a contract, when in fact runner.py imports ase.md.verlet, ase.optimize.FIRE and
ase.io.trajectory directly. Measured facts made that the right call rather than a
temporary one: PRRS_STATUS.md section 2.3 found no throughput advantage for the OpenMM
route, and prrs/openmm_backend.py earns its place as a parity checker for
tests/test_backend_parity.py, not as a second execution engine.

Nothing here is wrong. If a second dynamics backend is ever needed, this is the design to
start from -- the units contract, the capability declaration and the persist-it-in-the-
manifest rule are all still what the project wants. Until then it is a document.

Do not import this file. The live fail-closed exception is prrs.reliability.GateRejected.

--- original module below, unchanged ---
